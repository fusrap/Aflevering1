...
def get_user_cmd():
    while True:
        cmd = input('Indtast operation \'Random\',\'Add\' eller \'Subtract\': ').strip().lower().capitalize()
...
def process_operation(parameters):
        op = Operation(parameters[0], float(parameters[1]), float(parameters[2]))
        return json.dumps({
            "command": op._command,
            "number1": op._number1,
            "number2": op._number2
